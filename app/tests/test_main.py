...
from app.schemas.vapi_schema import (
    Message,
    ToolCall,
    ToolCallFunction,
    VapiRequest,
...
def test_create_user_success():
    request_data = create_vapi_request(
        "createUser", {"name": "New User", "phone_number": "9876543210"}
    )
...
def test_create_user_duplicate(db_session, test_user):
    request_data = create_vapi_request(
        "createUser",
        {"name": "Duplicate User", "phone_number": "1234567890"},
    )
...
def test_create_todo_success(db_session, test_user):
    ...
    response = client.post(
        "/schedules/create_todo/", json=request_data.model_dump()
    )
    assert response.status_code == 200
    assert response.json()["results"][0]["result"] == "success"
...
def test_get_todos_success(db_session, test_user, test_todo):
    request_data = create_vapi_request(
        "getTodos", {"phone_number": "1234567890"}
    )

    response = client.post(
        "/schedules/get_todos/", json=request_data.model_dump()
    )
    assert response.status_code == 200
...
def test_get_todos_user_not_found():
    request_data = create_vapi_request(
        "getTodos", {"phone_number": "0000000000"}
    )
...
def test_complete_todo_success(db_session, test_user, test_todo):
    ...
    request_data = create_vapi_request(
        "completeTodo",
        {"phone_number": "1234567890", "title": "Test Todo"},
    )

    response = client.post(
        "/schedules/complete_todo/", json=request_data.model_dump()
    )
    assert response.status_code == 200
    assert response.json()["results"][0]["result"] == "success"
    ...
    db_todo = (
        db_session.query(Todo).filter(Todo.id == test_todo.id).first()
    )
...
def test_complete_todo_not_found(db_session, test_user):
    request_data = create_vapi_request(
        "completeTodo",
        {"phone_number": "1234567890", "title": "Non-existent Todo"},
    )
...
def test_delete_todo_success(db_session, test_user, test_todo):
    request_data = create_vapi_request(
        "deleteTodo", {"phone_number": "1234567890", "title": "Test Todo"}
    )

    response = client.post(
        "/schedules/delete_todo/", json=request_data.model_dump()
    )
    assert response.status_code == 200
